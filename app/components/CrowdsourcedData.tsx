  ...
  return (
    <Card className="bg-gray-900 border-gray-800">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="text-white">{t("crowdsourcedData")}</CardTitle>
      ...
      <CardContent>
          ...
          {stats.map((stat, index) => (
            ...
            >
              ...
              <div className={`text-sm ${stat.change > 0 ? "text-green-500" : "text-red-500"}`}>
                {stat.change > 0 ? "+" : ""}
                {stat.change}% {selectedTimeFilter.toLowerCase()}
              </div>
            </motion.div>
          ))}
        </div>
        ...
        >
          ...
          <p>
            Incidents have increased by {Math.abs(stats[2].change)}% in the {selectedTimeFilter.toLowerCase()}. Stay
