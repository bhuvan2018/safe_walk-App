...
export default function Community({ onIncidentSubmit }: CommunityProps) {
  ...
  const handleSubmit = (type: "story" | "incident") => {
    ...
    setTimeout(() => {
      ...
      // Save to communityReports for the community section
      localStorage.setItem("communityReports", JSON.stringify(updatedReports))

      // Also save to adminReports for the admin dashboard
      const adminReportsString = localStorage.getItem("adminReports")
      const adminReports = adminReportsString ? JSON.parse(adminReportsString) : []
